...
PRETRAIN_VERSION=base
FINETUNE_VERSION=base-lora-zero2-r128 #experiment name for recording different runnings
TRAIN_RECIPE=lora #training recipes, other options are: lora, qlora
...
bash scripts/train/lora/finetune_lora.sh "$FINETUNE_DATA_PATH" "$FINETUNE_IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$PRETRAIN_VERSION" "$FINETUNE_VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
