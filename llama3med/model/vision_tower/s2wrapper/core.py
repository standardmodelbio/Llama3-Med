...
from .utils import merge_chessboard, split_chessboard
...
def forward(
    ...
    assert (
        scales is not None or img_sizes is not None
    ), "Please assign either scales or img_sizes."
    ...
    for size, num_split in zip(img_sizes, num_splits):
        x = F.interpolate(input.to(torch.float32), size=size, mode="bicubic").to(
            input.dtype
        )
    ...
    out = torch.cat(
        [
            F.interpolate(
                outs_multiscale[i].to(torch.float32), size=output_size, mode="area"
            ).to(outs_multiscale[i].dtype)
